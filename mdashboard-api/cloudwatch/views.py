...
from pytz import timezone as pytz_timezone  # Import pytz's timezone if needed
...
def log_count_interval(request):
    ...
    # Setting timezone to Nepal Standard Time
    ...
    interval_mapping = {
        'last_hour': {'delta': timedelta(minutes=5)},
        'last_day': {'delta': timedelta(hours=1)},
        'last_week': {'delta': timedelta(days=1)},
        'last_month': {'delta': timedelta(days=1)}
    }

    if interval_type not in interval_mapping:
        return Response({"error": "Invalid interval type. Valid options are 'last_hour', 'last_day', 'last_week', and 'last_month'."},
                        status=status.HTTP_400_BAD_REQUEST)

    interval_info = interval_mapping[interval_type]
    interval_delta = interval_info['delta']
    ...
    if interval_type == 'last_hour':
        ...
        end_time = now.replace(hour=0, minute=0, second=0, microsecond=0) - timedelta(days=now.weekday() + 1)
    ...
    if period:
        try:
            period_start_time, period_end_time = get_time_interval(period)
            filters &= Q(timestamp__range=(period_start_time, period_end_time))
        except ValueError as e:
            return Response({"error": str(e)}, status=status.HTTP_400_BAD_REQUEST)
    if securityinfo:
        if securityinfo not in ['INFO', 'ERROR', 'WARN']:
    ...
    # Fetch logs
    logs = Log.objects.filter(filters).order_by('timestamp')

    # Prepare response data
    response_data = []
    interval_time = start_time
    while interval_time < end_time:
        next_interval_time = interval_time + interval_delta
        count = logs.filter(timestamp__gte=interval_time, timestamp__lt=next_interval_time).count()
        response_data.append({
            'interval': interval_time.isoformat(),
            'count': count
        })
